#!/usr/bin/env python3
"""Train MLP with settlement stats as global features.

Features per cell:
  0-13: same local features as cross_mlp (terrain, neighbors, BFS dist, etc.)
  14: avg_population of settlements in viewport (normalized)
  15: n_unique_owners / 10 (faction diversity)
  16: settlement_survival_rate (alive/total)
  17: avg_defense (normalized)
  18: total_population / 100 (growth intensity)

Target: 6-class probability distribution from ground truth.

The key insight: settlement stats reflect HIDDEN game parameters (lambda, mu_spawn)
that can't be derived from the initial grid. Unlike grid-based global features
(settle_density, forest_ratio) which overfitted to specific round layouts.
"""
...
REPLAY_DIR = "../astar-island"  # simulations/ and replay dirs are under here
NUM_CLASSES = 6
LOCAL_FEAT_DIM = 14
GLOBAL_FEAT_DIM = 5
...
def extract_local_features(grid):
    """Same local features as train_cross_mlp.py"""
    H, W = grid.shape
...
def extract_settlement_stats_from_replays(round_id, seed_idx):
    """Load replays for a round/seed and compute average settlement stats."""
    # Find replay files
    sim_dirs = glob.glob(os.path.join(REPLAY_DIR, "simulations", "*"))

    replay_files = []
    for sd in sim_dirs:
        # Check if this simulation belongs to this round
        summary_path = os.path.join(sd, "summary.json")
        if os.path.exists(summary_path):
            with open(summary_path) as f:
                summary = json.load(f)
            if summary.get("round_id", "")[:8] == round_id[:8]:
                # Look for seed replay file
    ...
    for rf in replay_files:
        ...
        # Handle both formats: list of query results or replay format
        if isinstance(data, list):
            # Query results format: [{viewport, response}, ...]
            for item in data:
                ...
                if settlements:
                    ...
                    if alive:
                        stats = {
                            ...
                            'n_owners': len(set(s.get('owner_id', 0) for s in alive)),
                            'survival': len(alive) / max(1, len(settlements)),
                            'avg_def': np.mean([s.get('defense', 0) for s in alive]),
    ...
    # Average across replays
    return {
        'avg_pop': np.mean([s['avg_pop'] for s in all_stats]),
        'n_owners': np.mean([s['n_owners'] for s in all_stats]),
        'survival': np.mean([s['survival'] for s in all_stats]),
        'avg_def': np.mean([s['avg_def'] for s in all_stats]),
        'total_pop': np.mean([s['total_pop'] for s in all_stats]),
    }


def collect_data():
    """Collect training data with settlement stats as global features."""
    X_all, Y_all = [], []
    ...
    # Cache settlement stats per round
    settle_stats_cache = {}
    stats_found = 0
    stats_missing = 0
    ...
    for af in analysis_files:
        ...
        # Get settlement stats (cached per round)
        cache_key = (round_id[:8], seed_idx)
        if cache_key not in settle_stats_cache:
            settle_stats_cache[cache_key] = extract_settlement_stats_from_replays(round_id, seed_idx)

        settle_stats = settle_stats_cache[cache_key]
        if settle_stats is None:
            stats_missing += 1
            # Use default/neutral values
            global_feats = np.array([1.0, 0.5, 1.0, 0.3, 50.0], dtype=np.float32)
        else:
            stats_found += 1
            global_feats = np.array([
                min(settle_stats['avg_pop'], 5.0) / 5.0,      # avg pop normalized
                settle_stats['n_owners'] / 10.0,                # faction diversity
                settle_stats['survival'],                       # survival rate
                min(settle_stats['avg_def'], 1.0),              # defense normalized
                min(settle_stats['total_pop'], 500.0) / 500.0,  # total pop normalized
            ], dtype=np.float32)
        ...
        try:
            ...
            for y in range(H):
                for x in range(W):
                    t = int(grid[y, x])
                    c = terrain_to_class(t)
                    if c == 5 or t == 10:  # mountain/ocean skip
                        continue
    ...
    print(f"Settlement stats: {stats_found} found, {stats_missing} missing")
...
class MLP:
    ...
    def export_cpp(self, path, prefix="settle"):
        P = prefix.upper()
        p = prefix.lower()
        with open(path, 'w') as f:
            f.write("#pragma once\n")
            f.write("// Auto-generated settlement-stats MLP weights\n\n")
...
def main():
    ...
    print("Collecting training data with settlement stats...")
    X, Y = collect_data()
    print(f"Dataset: {X.shape[0]} cells, {X.shape[1]} features, {Y.shape[1]} classes")

    # Also train baseline (local features only) for comparison
    ...
    # Train with settlement features
    print(f"\n=== Training with settlement features ({FEAT_DIM}D) ===")
    ...
    for k, w in zip(model._params, best_weights):
        setattr(model, k, w)
    print(f"Best val loss (with settle stats): {best_val:.4f}")

    # Train baseline for comparison
    print(f"\n=== Training baseline ({LOCAL_FEAT_DIM}D) ===")
    baseline = MLP(LOCAL_FEAT_DIM, HIDDEN, NUM_CLASSES)
    baseline.weight_decay = 1e-4
    best_base_val = float('inf')
    best_base_weights = None

    # Need to re-shuffle since we modified X_train above
    X_train_local = X_local[idx[:split]]
    Y_train_base = Y[idx[:split]].copy()
    Y_val_base = Y[idx[split:]]

    for epoch in range(300):
        perm = np.random.permutation(len(X_train_local))
        X_train_local = X_train_local[perm]
        Y_train_base = Y_train_base[perm]
        lr = 0.001 * (1.0 - 0.5 * epoch / 300)
        train_loss = 0
        n = 0
        for i in range(0, len(X_train_local), 256):
            loss = baseline.train_step(X_train_local[i:i+256], Y_train_base[i:i+256], lr)
            train_loss += loss
            n += 1
        train_loss /= n
        val_probs, _, _ = baseline.forward(X_val_local)
        eps = 1e-8
        val_loss = np.mean(np.sum(Y_val_base * np.log((Y_val_base + eps) / (val_probs + eps)), axis=1))
        if val_loss < best_base_val:
            best_base_val = val_loss
            best_base_weights = tuple(getattr(baseline, k).copy() for k in baseline._params)
        if epoch % 20 == 0:
            print(f"  Epoch {epoch}: train={train_loss:.4f} val={val_loss:.4f}")

    print(f"\nBest val loss (baseline 14D):     {best_base_val:.4f}")
    print(f"Best val loss (with settle 19D):  {best_val:.4f}")
    print(f"Improvement: {best_base_val - best_val:.4f}")

    if best_val < best_base_val:
        print("\nSettlement features HELP! Exporting...")
        model.export_cpp("settle_mlp_weights.h", "settle")
        print("Exported to settle_mlp_weights.h")
    else:
        print("\nSettlement features don't help (or hurt). Not exporting.")
